    ...
    for (int c = 0; c < channels_; ++c) {
      for (int ph = 0; ph < pooled_height_; ++ph) {
        for (int pw = 0; pw < pooled_width_; ++pw) {
          ...
          int hstart = (ph * roi_height) / pooled_height_;
          if ( (hstart * pooled_height_) > (ph * roi_height) ) {
            --hstart;
          }
          int wstart = (pw * roi_width) / pooled_width_;
          if ( (wstart * pooled_width_) > (pw * roi_width) ) {
            --wstart;
          }
          int hend = ((ph + 1) * roi_height) / pooled_height_;
          if ( (hend * pooled_height_) < ((ph + 1) * roi_height) ) {
            ++hend;
          }
          int wend = ((pw + 1) * roi_width) / pooled_width_;
          if ( (wend * pooled_width_) < ((pw + 1) * roi_width) ) {
            ++wend;
          }
