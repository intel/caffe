...
#include "caffe/new_data_transformer.hpp"
#include "caffe/layers/new_base_data_layer.hpp"
#include "caffe/layers/new_video_data_layer.hpp"
#include "caffe/util/new_benchmark.hpp"
#include "caffe/util/new_io.hpp"
#include "caffe/util/new_image_io.hpp"
#include "caffe/util/new_math_functions.hpp"
#include "caffe/util/new_rng.hpp"

namespace caffe {

template <typename Dtype>
NewVideoDataLayer<Dtype>::~NewVideoDataLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void NewVideoDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const int new_length = this->layer_param_.new_video_data_param().new_length();
  const int new_height = this->layer_param_.new_video_data_param().new_height();
  const int new_width  = this->layer_param_.new_video_data_param().new_width();
  string root_folder = this->layer_param_.new_video_data_param().root_folder();
  ...
  // Read the list file
  const string& source = this->layer_param_.new_video_data_param().source();
  const bool use_temporal_jitter = this->layer_param_.new_video_data_param().use_temporal_jitter();
  const bool use_image = this->layer_param_.new_video_data_param().use_image();
  const int sampling_rate = this->layer_param_.new_video_data_param().sampling_rate();
  const bool use_multiple_label = this->layer_param_.new_video_data_param().use_multiple_label();
  if (use_multiple_label) {
    CHECK(this->layer_param_.new_video_data_param().has_num_of_labels()) <<
    "number of labels must be set together with use multiple labels";

  }
  const int num_of_labels = this->layer_param_.new_video_data_param().num_of_labels();
  ...
  if (this->layer_param_.new_video_data_param().shuffle()) {
  ...
  if (this->layer_param_.new_video_data_param().rand_skip()) {
    unsigned int skip = caffe_rng_rand() %
        this->layer_param_.new_video_data_param().rand_skip();
  ...
  const int batch_size = this->layer_param_.new_video_data_param().batch_size();
...
template <typename Dtype>
void NewVideoDataLayer<Dtype>::ShuffleClips() {
...
template <typename Dtype>
void NewVideoDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  NewVideoDataParameter new_video_data_param = this->layer_param_.new_video_data_param();
  const int batch_size = new_video_data_param.batch_size();
  const int new_length = new_video_data_param.new_length();
  const int new_height = new_video_data_param.new_height();
  const int new_width = new_video_data_param.new_width();
  string root_folder = new_video_data_param.root_folder();
  const bool use_image = new_video_data_param.use_image();
  const bool use_temporal_jitter = new_video_data_param.use_temporal_jitter();
  int sampling_rate = new_video_data_param.sampling_rate();
  const int max_sampling_rate = new_video_data_param.max_sampling_rate();
  const bool use_sampling_rate_jitter = new_video_data_param.use_sampling_rate_jitter();
  const bool show_data = new_video_data_param.show_data();
...
INSTANTIATE_CLASS(NewVideoDataLayer);
REGISTER_LAYER_CLASS(NewVideoData);
