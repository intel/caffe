...
# number of nodes is determined by slurm
...
# slurm host file
scontrol show hostnames $SLURM_NODELIST > /tmp/host_file
# a list of nodes
host_file=/tmp/host_file
...
function usage
{
    ...
    echo ""
    echo "  Parameters:"
    echo "    hostfile: host file includes list of nodes. Only used if you're running with multinode. With slurm system, you don't need to set it"
    ...
    echo "    benchmark: none(disabled by default). Includes all-reduce performance"
    echo "      Dependency: user needs to install Intel MPI library (including IMB-MPI1);"
    echo "                  and add it in system path."
...
function execute_command
{
    ...
    if [ ${numnodes} -eq 1 ] && [ $is_local_run -eq 1 ]; then
        ...
        time GLOG_minloglevel=0 mpiexec.hydra -bootstrap slurm $exec_command 2>&1 | tee ${log_file}
    fi
